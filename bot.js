...
app.get("/", (request, response) => {//HyBot
...
fs.readdir("./komutlar/", (err, files) => {//HyBot
...
client.reload = command => {
  return new Promise((resolve, reject) => {//HyBot
    try {
...
client.elevation = message => {//HyBot
  if (!message.guild) {
    return;
  }
  let permlvl = 0;
...
client.on("ready", () => {//HyBot
  client.user.setPresence({
    game: { name: `Sizi`, type: "WATCHING" },
    status: "online"
  });
});
...
client.on(`guildMemberAdd`, async member => {//HyBot
...
client.on('guildMemberAdd', async member => {//s
if(member.user.bot)
member.setRoles(['729981065644474490'])
})
// GİRİŞ 
  client.on("guildMemberAdd", member => { 
